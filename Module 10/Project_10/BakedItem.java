...
public abstract class BakedItem implements Comparable<BakedItem> {
   ...
   */
   ...
   /**
      Public method that compares two baked item objects to each other.
      Returns <0 if object is less than the one being compared, returns 0 if
      they are equal, and returns >0 if the object is greater than the one
      being compared.
      
      @param itemIn - Accepts BakedItem input for the item being compared.
      @return int - Returns an integer value dependet on if the items being
      compared are less than, greater than, or equal to each other.
   */
   public int compareTo(BakedItem itemIn) {
   
      return toString().toLowerCase().compareTo(itemIn.toString()
         .toLowerCase());
